...
6. **Verify it like a skeptic.** Run the item's verify steps end-to-end as a real user would. Evidence, not assertions: capture the test output, the command and what it returned, or the screenshot. Good-looking results are suspicious — if something passes too easily, hunt for the bug before trusting it. Go to the source of truth (the DB row, the raw API response, the file on disk), never a UI badge or your own memory of "I did that." For anything with a UI: open the real app and click every button and flow you touched — no dead ends, works on phone and desktop, matches the existing design system.
